...
contract VestingController {
    ...
    struct VestingInvestor {
        ...
        uint256 totalInvestmentCounter;
        mapping(uint256 => VestingInvestment) VestingInvestments;
    }

    // Just to try logic, needs initializer implementation for upgradability
    constructor() {}

    function getInvestment(address _investor, uint256 _investment)
        public
        view
        returns (VestingInvestment memory)
    {
        VestingInvestment memory investment = vestingInvestor[_investor]
            .VestingInvestments[_investment];
        return investment;
    }
    ...
    function addInvestment(
        ...
        uint256 dailyTokenAmount = _tokenAmount / _vestingPeriod;
        ...
        uint256 total = vestingInvestor[_recipient].totalInvestmentCounter;
        vestingInvestor[_recipient].totalInvestmentCounter =
            vestingInvestor[_recipient].totalInvestmentCounter +
            1;
        vestingInvestor[_recipient].VestingInvestments[total] = investment;
        vestingInvestor[_recipient].totalTokenAmount =
            vestingInvestor[_recipient].totalTokenAmount +
            _tokenAmount;
    }
    ...
    function calculateLockedAmount(address _investorAddress)
        public
        view
        returns (uint256 _lockedAmount)
    {
        uint256 total = vestingInvestor[_investorAddress]
            .totalInvestmentCounter;
        uint256 lockedAmount = 0;
        uint256 locked;

        // Loop through investments for the investor
        for (uint256 i = 0; i < total; i++) {
            VestingInvestment memory investment = vestingInvestor[
                _investorAddress
            ].VestingInvestments[i];
