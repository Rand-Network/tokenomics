...
contract VestingController is ERC721, ERC721Enumerable {
	...
	function addInvestment(address recipient,
							uint8 tokenId,
							uint256 tokenAmount,
							uint8 vestingPeriod,
							uint8 vestingStartTime,
		...
		VestingInvestment memory investment = VestingInvestment(tokenId,
																tokenAmount,
																vestingPeriod,
																vestingStartTime,
																cliffStartTime,
	...
	function _calculateTotalLockedAmount(address investor) internal view returns (uint256 totalLockedAmount) {
	...
	function _calculateLockedAmount(uint tokenId) internal view returns (uint256 lockedAmount) {
		...
		if (vestedPeriods <= investment.vestingPeriod) {
			uint locked = investment.tokenAmount - (vestedPeriods * investment.dailyTokenAmount);
		}
		else {
			locked = 0;
		}	
